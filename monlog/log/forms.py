...
class RelativedeltaWidget(forms.MultiWidget):
    ...
    def _has_changed(self, initial, data):
        initial = (initial.months, initial.days, initial.hours, initial.minutes, initial.seconds)
        return initial == data
...
class RelativedeltaField(forms.Field):
    ...
    def prepare_value(self, value):
        if value is not None:
            ...
            return (value.months, value.days, value.hours, value.minutes, value.seconds)
        else:
            return None

    def clean(self, values):
        (months, days, hours, minutes, seconds) = [int(v) for v in values]
        return relativedelta(months=months, days=days, hours=hours, minutes=minutes, seconds=seconds)

class ExpectationForm(forms.ModelForm):
    """
    This is the form for expectations
    """
    expectation_name = forms.CharField(widget=forms.TextInput(attrs={'placeholder':'Enter an expectation name...'}))
    ...
    search = forms.CharField(required=False, max_length=100,
                     widget=forms.TextInput(attrs={'class':'search-query', 'placeholder':'Search...'}))
    severity__in = forms.MultipleChoiceField(required=False, widget=SelectMultiple, choices=SEVERITY_CHOICES)
    application__in = forms.MultipleChoiceField(required=False, widget=SelectMultiple, choices=())
    server_ip__in = forms.MultipleChoiceField(required=False, widget=SelectMultiple, choices=())
